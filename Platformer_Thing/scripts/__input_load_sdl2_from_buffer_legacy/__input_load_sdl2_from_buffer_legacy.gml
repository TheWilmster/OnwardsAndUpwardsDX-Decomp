function __input_load_sdl2_from_buffer_legacy(arg0)
{
	...
	var _size = buffer_get_size(arg0);
		...
		else
		{
			...
			if (!_in_string)
			{
				...
				if (_read || _value == _cell_delimiter_ord)
				{
					...
					buffer_seek(arg0, buffer_seek_start, _word_start);
					...
					_string = string_replace_all(_string, _string_delimiter_double, _string_delimiter);
			...
			if (_guid != "" && string_pos("#", _guid) <= 0)
			{
				...
				if (_platform == undefined || _platform == os_type)
				{
					...
					if (_description != "")
					{
						...
						{
							_description_array = [];
							variable_struct_set(_db_by_description, _description, _description_array);
						}
