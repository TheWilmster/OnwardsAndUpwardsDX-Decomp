...
void main()     //  Where the magic happens
{
    ...
    val = max(val,(float(colIntensity>0.825)*4.0));    //Using 'max' means that if we already have a value it won't clear the color we've already set when the comparisons are false.
